...
function DispatchModal({ dispatch, vehicles, availability, journalists, cameramen, assistants, directors, personnelOnDuty, liveuEquipment, activeDispatches = [], onSave, onClose }) {
  ...
  const usedCameramanIds = new Set(activeDispatches.filter(d => d.id !== currentDispatchId && d.cameraman_id).map(d => d.cameraman_id));
  const usedAssistantIds = new Set(activeDispatches.filter(d => d.id !== currentDispatchId && d.assistant_id).map(d => d.assistant_id));
  const usedDirectorIds = new Set(activeDispatches.filter(d => d.id !== currentDispatchId && d.director_id).map(d => d.director_id));
  const usedLiveuIds = new Set(activeDispatches.filter(d => d.id !== currentDispatchId && d.liveu_id).map(d => d.liveu_id));
  const usedJournalistIds = new Set(activeDispatches.filter(d => d.id !== currentDispatchId && d.journalist_id).map(d => d.journalist_id));
