...
export const RouteGroupsModal = ({ isOpen, onClose, shiftType }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (selectedGroup) {
        await routeGroupsService.update(selectedGroup.id, formData);
      } else {
        await routeGroupsService.create(formData);
      }
      await loadData();
      setIsCreating(false);
      setSelectedGroup(null);
      setFormData({ name: '', description: '', shift_type: shiftType || 'AM', personnel_ids: [] });
    } catch (err) {
  ...
  const handleDelete = async (groupId) => {
    ...
    setLoading(true);
    try {
      await routeGroupsService.delete(groupId);
      await loadData();
      setSelectedGroup(null);
      setIsCreating(false);
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-purple-600 text-white px-6 py-4 flex justify-between items-center flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold">Grupos de Rutas — Turno {shiftType}</h2>
          </div>
          <button onClick={() => onClose(null)} className="text-white hover:bg-purple-700 p-2 rounded">
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-hidden flex min-h-0">
          ...
          <div className="w-72 border-r overflow-y-auto p-4 flex-shrink-0">
            <button
              ...
              disabled={loading}
              className="w-full mb-4 flex items-center justify-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 font-medium"
            >
              ...
              {groups.map(group => (
                <div
                  ...
                  className={`p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors ${
                    selectedGroup?.id === group.id ? 'bg-purple-50 border-purple-400' : 'border-gray-200'
                  }`}
                  onClick={() => handleSelectGroup(group)}
                >
                  <div className="font-semibold text-gray-900">{group.name}</div>
                  <div className="text-sm text-gray-500 mt-1">
                    {group.personnel_ids?.length || 0} personas · {group.shift_type}
                  </div>
                  {group.description && (
            ...
            {groups.length > 0 && !isCreating && !selectedGroup && (
              <div className="mt-6 border-t pt-4">
                <p className="text-xs text-gray-500 mb-2 font-medium uppercase">Cargar en rutas:</p>
            ...
            {(isCreating || selectedGroup) ? (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  {selectedGroup ? `Editando: ${selectedGroup.name}` : 'Nuevo Grupo'}
                </h3>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                  >
                    <option value="AM">AM — Mañana (05:00)</option>
                    <option value="PM">PM — Noche (22:00)</option>
                  </select>
                </div>
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <div className="text-xs text-gray-500 truncate">
                                      {person.localidad || person.barrio || ''}{person.direccion ? ` · ${person.direccion}` : ''}
                                    </div>
                                  </div>
                                </div>
                              );
              ...
              <div className="text-center text-gray-400 py-16">
                ...
                <p className="text-sm mt-1">o crea uno nuevo con el botón de la izquierda</p>
              </div>
            )}
          </div>
        </div>
